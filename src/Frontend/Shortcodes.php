...
class Shortcodes {
     ...
     *
     * Usage: [treatment_packages category="women-packages" columns="3"]
     *
    ...
    public static function render_treatment_packages( $atts ) {
        ...
        $categories = TreatmentTaxonomies::get_categories( array( 'hide_empty' => true ) );

        // Determine active category
        $active_category = ! empty( $atts['category'] ) ? $atts['category'] : '';

        // If no category specified and we have categories, use the first one
        if ( empty( $active_category ) && ! empty( $categories ) && ! is_wp_error( $categories ) ) {
            $active_category = $categories[0]->slug;
        }

        // Get treatments
        $treatments = self::get_treatments( $active_category, $atts );

        ob_start();
        ?>
        <div class="tp-packages-wrapper" data-columns="<?php echo esc_attr( $atts['columns'] ); ?>">
            ...
            <div class="tp-packages-container">

                <?php if ( 'yes' === $atts['show_sidebar'] && ! empty( $categories ) && ! is_wp_error( $categories ) ) : ?>
    ...
    public static function render_single_treatment( $atts ) {
        ...
        $treatment = get_post( $atts['id'] );

        if ( ! $treatment || TreatmentPostType::POST_TYPE !== $treatment->post_type ) {
            return '';
        }
